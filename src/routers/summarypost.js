...
router.get("/getall", async (req, res) => {
  try {
    req.query.semesterFilter = !req.query.semesterFilter
      ? Array.from(Array(await Semester.count()), (_, i) => i + 1)
      : parseInt(req.query.semesterFilter);
    req.query.pageNumber = !req.query.pageNumber ? 1 : req.query.pageNumber;
    const sortby = [
      req.query.sortBy ? req.query.sortBy : "summaryPostId",
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const summaryPostId = req.params.summaryPostId;
      const uploadFile = await SummaryPost.update(
        { blobFile },
        {
          where: {
            summaryPostId,
...
router.put("/edit/:summaryPostId", auth, async (req, res) => {
  try {
    const summaryPostId = req.params.summaryPostId;
    ...
    if (updatedSummaryPost[0] === 0) {
      return res.status(400).send({
        error: "summaryPost not found or noting change in summaryPosts!",
      });
    }
...
router.delete("/delete/:summaryPostId", async (req, res) => {
  try {
    ...
    if (deletedSummaryPost === 0) {
      return res.status(400).send({
        error: "summaryPost not found or noting delete in summaryPosts!",
      });
    }
